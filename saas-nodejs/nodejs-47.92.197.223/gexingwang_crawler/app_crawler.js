...
function setMasterExt(master_id, master_type, master_website, cote_id, cote_name, master_text, master_href, UploadID, flyDate, CoteID) {
    ...
    .end(function(err, sres) {
        ...
        var json = JSON.parse(sres.text);
        var res = json[1].res;
        console.log(master_id + "->" + res.length);
    ...
    connection.query(app_sql.queryCrawlerDetail, [],
        function(results, fields) {
            ...
            results.forEach(function(element, index) {
                //crawlerDetail(element.detail_id, element.master_id, element.cote_id, element.cote_name, element.master_text, element.master_href, element.detail_crawler_page, element.detail_crawler_href);
...
function crawlerDetail(
    // detail_id, master_id, cote_id, cote_name, master_text, master_href, detail_page, detail_crawler_href
    detail_id, master_id, master_type, master_website, master_href, cote_id, cote_name, master_text, detail_crawler_page, detail_crawler_href
) {
        ...
        try {
            ...
            res.forEach(function(element, index) {
                ...
                if (rankList.indexOf(rank) < 0) {
                    ...
                    //var rt = [detail_id2, master_id, cote_id, cote_name, master_text, pageid, index + 1, '0', element.FlyTime, element.GBCode, element.MemberName, element.HomeDate, element.UploadIdx, element.Speed, rank, new Date(), new Date()];
