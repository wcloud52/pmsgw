...
router.get('/master', function(req, res, next) {
    ...
    var list = ['http://www.086019.com/live/index/p/1.html', 'http://www.086019.com/live/index/p/2.html', 'http://www.086019.com/live/index/p/3.html'];
    for (var ii = 0; ii < list.length; ii++) {
        crawlerMaster(list[ii]);
    }
...
function crawlerMaster(curPath) {
    superagent
        .get(curPath)
        .buffer(true)
...
function setMasterExt(master_id, master_type, master_website, cote_id, cote_name, master_text, master_href, master_number) {
    superagent
        ...
        .end(function(err, sres) {
            ...
            var totalCount =$("#guichaolv").parent().text().replace($("#guichaolv").text(),'');
            if(totalCount==0)
            totalCount=10000;
            console.log("setMasterExt->" + master_href + "->" + totalCount);
...
function crawlerDetail(detail_id, master_id, master_type, master_website, master_href,
    cote_id, cote_name, cote_state, master_text, detail_crawler_page, detail_crawler_href) {
