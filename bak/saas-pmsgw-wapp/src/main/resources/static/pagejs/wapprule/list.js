...
layui.use([ 'element', 'table', 'form',  'fuzzyQuery' ], function() {
	...
	baseOperation.initTable("listTable", "/wappRule/list", {
	...
	table.on('tool(test)', function(obj) {
		...
		if (obj.event === 'detail') {
			...
			baseOperation.viewForm(url, '规则定义数据', layui);
		} else if (obj.event === 'del') {
			var id=data.id;
			var url='/wappRule/delete/' + id;
			baseOperation.deleteForm(url ,layui, flashTable);
		} else if (obj.event === 'edit') {
			var id=data.id;
			var url='/wappRule/edit.html?id=' + id;
			var opUrl="/wappRule/update";
			baseOperation.editForm(url, '规则定义数据', opUrl, layui, flashTable);
		}
	});
	...
	$('#btnSet').on('click', function() {
		...
		if (data != null && data.length == 1) {
			...
			var url='/wappRule/detailList.html?id=' + id;
