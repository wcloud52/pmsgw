...
import com.ifp.weixin.entity.AccessTokenOAuth;
...
public class OAuthService {
	 ...
	 * weixin oauth url
	 */
	public static String OAUTH = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=APPID&redirect_uri=REDIRECT_URI&response_type=code&scope=SCOPE&state=STATE#weixin_redirect";
	...
	public static String getOauthUrl(String redirectUrl,String charset,String scope){
		...
		try {
			url = OAUTH
					.replace("APPID", ConstantWeixin.APPID)
	...
	public static AccessTokenOAuth getOAuthAccessToken(String code) {
		String url = GET_ACCESS_TOKEN_OAUTH
				.replace("APPID", ConstantWeixin.APPID)
				.replace("SECRET", ConstantWeixin.APPSECRET)
				.replace("CODE", code);
